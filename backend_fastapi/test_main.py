...
class ResearchTests(unittest.TestCase):
    def test_request_rejects_invalid_code(self):
        with self.assertRaises(Exception):
            main.ResearchRequest(code="123", name="测试")

    def test_evidence_prompt_keeps_ids(self):
        text = main.evidence_to_prompt([{
    ...
    @patch.object(main, "call_llm", return_value="## 报告\n事实 [BASIC-1]")
    @patch.object(main, "collect_evidence", return_value=([{
    ...
    def test_research_returns_auditable_payload(self, _collect, _llm):
        markdown, evidence, coverage, notes = main.research("000001", "测试")
        self.assertTrue(markdown.endswith(main.DISCLAIMER))
