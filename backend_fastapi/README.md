...
**默认下载窗口是「最近 5 年」，用户可改**：界面「起始日期」默认填的是「5 年前的今天」
（按当前日期算，服务常年不重启也不会把窗口拖短）。系统里最长的需求是 MA60（60 交易日）与
RPS（约 250 交易日），5 年绰绰有余；想要更长的历史，把起始日改成 `1990-01-01` 即可拿到
上市至今（数据源会自动截断到上市首日）。容量提示与磁盘检查都按实际年数缩放，不会虚报。
起始日晚于 3 年分界点时只有一个阶段，不再排「更早历史」那一轮。
...
- **后台闲时下载**：低并发（1 或 2）接着上次没下完的股票继续补，已跑过的不再重排。
